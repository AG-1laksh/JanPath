...
export default function AdminLogin() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    // Mock validation
    // In a real app, you might want to allow other admins, but for now we keep the check if strictly required.
    // Reducing strictness slightly for "demo" purposes or checking against list of admins if auth succeeds.
    if (email.trim().toLowerCase() !== ADMIN_EMAIL) {
      // Proceeding with auth generally, but if specific restrictions needed:
    ...
    try {
      ...
      // Additional check if needed after auth
      if (email.trim().toLowerCase() !== ADMIN_EMAIL) {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-[#050505] p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-5xl grid md:grid-cols-2 gap-12 items-center relative z-10">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-xs font-medium text-purple-200 mb-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="p-4 rounded-2xl bg-white/5 border border-white/5 backdrop-blur-sm">
              <div className="text-2xl font-semibold text-white mb-1">Secure</div>
              <div className="text-sm text-slate-500">End-to-End Encrypted</div>
            </div>
          </div>
        </motion.div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="space-y-2">
                  ...
                  <div className="relative group/input">
                    <Mail className="absolute left-4 top-3.5 text-slate-500 group-focus-within/input:text-purple-400 transition-colors" size={18} />
                    <input
                      type="email"
                      value={email}
                      onChange={(e) => setEmail(e.target.value)}
                      placeholder="admin@janpath.gov"
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-12 py-3 text-white placeholder:text-slate-600 focus:outline-none focus:border-purple-500/50 focus:bg-white/10 transition-all"
                      required
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group/input">
                    <Lock className="absolute left-4 top-3.5 text-slate-500 group-focus-within/input:text-purple-400 transition-colors" size={18} />
                    <input
                      type="password"
                      value={password}
